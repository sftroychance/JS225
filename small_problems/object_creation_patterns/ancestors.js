// Implement an ancestors method that returns the prototype chain (ancestors) of
// a calling object as an array of object names. Here's an example output:

// A:
// recursively iterate over object prototypes until you get to Object.prototype
// (base case)

// name property added to make objects easier to identify
const foo = {name: 'foo'};
const bar = Object.create(foo);
bar.name = 'bar';
const baz = Object.create(bar);
baz.name = 'baz';
const qux = Object.create(baz);
qux.name = 'qux';

// method is being called on the objects, so we need to add this method to the
// foo object

foo.ancestors = function ancestors(prototypeChain = []) {
  const ancestor = Object.getPrototypeOf(this);

  if (ancestor === Object.prototype) {
    prototypeChain.push('Object.prototype');
    return prototypeChain;
  } else {
    prototypeChain.push(ancestor.name);
    return ancestor.ancestors(prototypeChain);
  }
};

console.log(qux.ancestors());
// returns ['baz', 'bar', 'foo', 'Object.prototype']
console.log(baz.ancestors());
// returns ['bar', 'foo', 'Object.prototype']
console.log(bar.ancestors());
// returns ['foo', 'Object.prototype']
console.log(foo.ancestors());
// returns ['Object.prototype']
